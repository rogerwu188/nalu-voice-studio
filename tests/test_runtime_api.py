...
def test_project_rename_archive_export_and_restore(tmp_path: Path) -> None:
    ...
    legacy["payload"].pop("production_runs")
    legacy["payload"].pop("publication_reconciliations")
    legacy["payload"].pop("publication_metric_snapshots")
    legacy["payload"].pop("director_strategy_revisions")
...
def test_database_migration_preserves_existing_database(tmp_path: Path) -> None:
    ...
    assert api.state.repository.db.schema_version() == 28
    with sqlite3.connect(database_path) as connection:
...
def test_populated_v1_database_upgrades_without_project_loss(tmp_path: Path) -> None:
    ...
    assert after.app.state.repository.db.schema_version() == 28
...
def test_project_season_and_episode_asset_scope_inheritance(tmp_path: Path) -> None:
    ...
    legacy_v3["payload"].pop("production_runs")
    legacy_v3["payload"].pop("publication_reconciliations")
    legacy_v3["payload"].pop("publication_metric_snapshots")
    legacy_v3["payload"].pop("director_strategy_revisions")
